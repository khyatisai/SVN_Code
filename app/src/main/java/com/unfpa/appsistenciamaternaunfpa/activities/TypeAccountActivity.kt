...
import com.unfpa.appsistenciamaternaunfpa.Mhserivice
...
import com.unfpa.appsistenciamaternaunfpa.fragments.introductory.*
import com.unfpa.appsistenciamaternaunfpa.utils.AppUtils
import com.unfpa.appsistenciamaternaunfpa.utils.Constant
import io.github.inflationx.viewpump.ViewPumpContextWrapper
import io.socket.client.Socket
import kotlinx.android.synthetic.main.activity_type_account.*
import org.json.JSONArray
import org.json.JSONObject

class TypeAccountActivity : AppCompatActivity() {
    ...
    private val ONESIGNAL_APP_ID = "7d29f60e-f3cb-4801-ab1b-24d26de231bb"
    ...
    override fun onResume() {
        ...
        AppUtils.trackScreen(Constant.USER_PROFILE_REG, this)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Start service
        if (!Mhserivice.IS_RUNNING) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                startForegroundService(Intent(applicationContext, Mhserivice::class.java))
            } else {
                startService(Intent(applicationContext, MainActivity::class.java))
            }
        }

//        try {
//            mSocket = IO.socket("https://chat-backend-unfpa.herokuapp.com")
...
//            if (intent != null && intent.getStringExtra(Mhserivice.KEY_DATA) != null) {
//                findViewById<TextView>(R.id.txtValueView).text = intent.getStringExtra(Mhserivice.KEY_DATA)
//            }


//        callBtn.setOnClickListener{
////            var intent = Intent(this, beforecall::class.java)
...
////        }
        ...
        if (isLoggedIn()) {
            ...
            if (typeUser == "paciente") {
                val intent = Intent(this, MainPregnantActivity::class.java)
                startActivity(intent)

            } else if (typeUser == "medico") {
                val intent = Intent(this, MainDoctorActivity::class.java)
                startActivity(intent)
            } else if (typeUser == "brigadista") {
                val intent = Intent(this, MainBrigadistaActivity::class.java)
                startActivity(intent)
        ...
        try {
            cardEmbarazada.setOnClickListener {
                var pregnant = Intent(this, LoginPregnantActivity::class.java)
                startActivity(pregnant)
            }

            cardBrigadista.setOnClickListener {
                var brigadist = Intent(this, LoginBrigadistaActivity::class.java)
                startActivity(brigadist)
            }

            cardPersonalSalud.setOnClickListener {
                var medico = Intent(this, LoginMedicoActivity::class.java)
                startActivity(medico)
            }

            cardNurse.setOnClickListener {
                var nurse = Intent(this, LoginNurseActivity::class.java)
                startActivity(nurse)
            }
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    public fun getItemList(): MutableList<Type>? {

        return itemlist
    }

    public fun getCountryList(): MutableList<TypeForCountry>? {
    ...
    fun getEducationList(): ArrayList<Type> {
        ...
        val eduLevel =
            arrayListOf<String>(*resources.getStringArray(R.array.education_level_intro_que))
    ...
    private fun addStoryToDatabase(unique_id: String, itemName: String) {
        try {
            val personalDetailsDAO =
                MhealthRoomDatabase.getAppDataBase(this.applicationContext)!!.personalDetailsDAO()
            val listData: List<PersonalDetailsEntity> = personalDetailsDAO?.getAllContent()
            val id: Int = listData.count()
            val incrementedID: String = (id + 1).toString()
            val storeArray = JSONArray()
            val childObj = JSONObject()
            childObj.put("unique_id", incrementedID)
            childObj.put("title", itemName)
            storeArray.put(childObj)
            val personalDetailsList: List<PersonalDetailsEntity> =
                Gson().fromJson(storeArray.toString(), Array<PersonalDetailsEntity>::class.java)
                    .toList()
            DBHelper.insertPersonalDetails(
                personalDetailsList,
                applicationContext!!
            )//.applicationContext!!
            /*if(incrementedID == "" ){
                DBHelper.updatePersonalDetails(personalDetailsList, applicationContext!!)//.applicationContext!!
            }else{
                DBHelper.insertPersonalDetails(personalDetailsList, applicationContext!!)//.applicationContext!!
            }*/
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    private fun launchHomeScreen() {
        ...
        startActivity(Intent(this@TypeAccountActivity, MainActivity::class.java))
        finish()
    }
    ...
    private fun checkModulesVisibility(CountryCode: String) {
        try {
            ...
            val field_enable_service_locator: String =
                countryCodeRequest.field_enable_service_locator
            val field_language: String = countryCodeRequest.field_language
            val field_health_management_export =
                countryCodeRequest.field_health_management_export.toString()
...
//        val data = initialData(getMyUserId, getMyUserId+"19")
