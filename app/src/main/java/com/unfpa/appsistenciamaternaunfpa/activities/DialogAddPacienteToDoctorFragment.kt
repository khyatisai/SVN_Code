...
//        }
        ...
        return inflater.inflate(R.layout.fragment_dialog_addpacientetodoctor, container,false)
                ...
                val req = JsonObjectRequest(
                    ...
                    Response.Listener { response ->
                        ...
                        if(obj.getString("code") == "duplicate"){
                            context?.let { it1 ->
                                AppUtils.createCustomToast(obj.getString("message"), it1, layoutToast,"warning")
                            }
                        }else{
                            var intent = Intent(activity, PacienteActivity::class.java)
                            startActivity(intent)
                            getActivity()?.finish()
                            context?.let { it1 ->
                                AppUtils.createCustomToast(obj.getString("message"), it1, layoutToast,"success")
                            }
                        }
                    },
