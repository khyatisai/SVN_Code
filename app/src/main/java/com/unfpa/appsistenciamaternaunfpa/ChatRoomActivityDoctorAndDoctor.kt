...
class ChatRoomActivityDoctorAndDoctor : AppCompatActivity(), View.OnClickListener {
    ...
    lateinit var chatRoomAdapterDoctorAndDoctor: ChatRoomAdapterDoctorAndDoctor

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_chatroom_doctor_and_doctor)
...
//            Toast.makeText(this, userName.toString() + roomName.toString(), Toast.LENGTH_LONG).show()
        ...
        chatRoomAdapterDoctorAndDoctor = ChatRoomAdapterDoctorAndDoctor(this, chatList);
...
//                    }
                    ...
                    for (i in 0 until message.length()) {
                        ...
                        if(sender.getString("id") == getMyUserId){
                            val messages = Message(sender.getString("firstname")+sender.getString("lastname"), message.getJSONObject(i).getString("message"), roomName2, MessageType.CHAT_MINE.index)
                            addItemToRecyclerView(messages)
                        }
                        if(sender.getString("id") != getMyUserId){
                            val  messages = Message(sender.getString("firstname")+sender.getString("lastname"), message.getJSONObject(i).getString("message"), roomName2, MessageType.CHAT_PARTNER.index)
                            addItemToRecyclerView(messages)
                        }
                    }
