...
class IntroductoryActivity : AppCompatActivity() {
    ...
    private fun addStoryToDatabase(unique_id: String, itemName: String) {
        try {
            val personalDetailsDAO = MhealthRoomDatabase.getAppDataBase(this.applicationContext)!!.personalDetailsDAO()
            ...
            val personalDetailsList: List<PersonalDetailsEntity> =
                Gson().fromJson(storeArray.toString(), Array<PersonalDetailsEntity>::class.java).toList()
